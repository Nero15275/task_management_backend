...
export async function getTasks(
  ...
  if (user.role === UserRole.MANAGER) {
    if (filters?.teamLeadId) {
      const teamMembers = await UserRepository.findByReportsTo(
        filters.teamLeadId,
      );
      const teamUserIds = [
        filters.teamLeadId,
        ...teamMembers.map((m: any) => m._id.toString()),
      ];
...
export async function updateTask(
  ...
  console.log("Assignee:", assignee?.username);

  let isDirectReport = assignee?.reportsTo?.toString() === user.sub;
  ...
  if (!hasPermission) {
    throw new AppError(
      "Forbidden: You do not have permissions to modify this task",
      HTTP_STATUS.FORBIDDEN,
    );
  }
  ...
  if (dto.assignedTo) {
    if (user.role === UserRole.EMPLOYEE) {
      throw new AppError(
        "Forbidden: Employees cannot reassign tasks",
        HTTP_STATUS.FORBIDDEN,
      );
    }
    if (user.role === UserRole.TEAM_LEAD) {
      const targetUser = await UserRepository.findById(
        dto.assignedTo.toString(),
      );
      const isTargetDirectReport = targetUser?.reportsTo
        ? targetUser?.reportsTo?.toString() === user.sub
        : false;
      const isTargetSelf = dto.assignedTo === user.sub;

      if (!isTargetDirectReport && !isTargetSelf) {
        throw new AppError(
          "Forbidden: Team Leads can only reassign to team members or self",
          HTTP_STATUS.FORBIDDEN,
        );
      }
    }
...
export async function deleteTask(
  ...
  if (!hasPermission) {
    throw new AppError(
      "Forbidden: You do not have permissions to delete this task",
      HTTP_STATUS.FORBIDDEN,
    );
  }
